...
const CuteRobot = () => {
  // 1. Explicitly type the refs as THREE elements
  const headRef = useRef<THREE.Group>(null);
  const leftArmRef = useRef<THREE.Mesh>(null);
  const rightArmRef = useRef<THREE.Mesh>(null);
  
  // Mouse Tracking & Breathing Animation
  useFrame((state) => {
    // 2. Safety Check: Only run if head exists
    if (headRef.current) {
    ...
    // 3. Safety Check: Only run if arms exist
...
export const Robot3D = () => {
  return (
    <div className="w-full h-[500px] relative cursor-none pointer-events-none">
